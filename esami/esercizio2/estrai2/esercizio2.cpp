...
int contiene(list *l2, int code, int res) {
    if (l2 == NULL) {
        return res;
    }
    ...
    return contiene(l2->next, code, res1);
}

list *estrai_aux(list *l1, list *l2) {
    if (l1 == NULL) {
        return NULL;
    }

    list *l = estrai_aux(l1->next, l2);

    list *l3 = new list;
    if (l1->code % 2 == 0) {
        l3->code = contiene(l2, l1->code, 0);
        l3->next = l;
    }
    else {
        l3->code = -1;
